...
TEST_CASE("1 Derive PrivateKey and PublicKey for ECDSA NID_X9_62_prime256v1", "[BTCKey]") {
	static int ECDSA_NID = NID_X9_62_prime256v1;
	...
	for (uint32_t index = 0; index < 10; index++) {
		...
		uint8_t data[] = {0, 1, 2, 3, 4, 5};
		CMBlock md;
		md.SetMemFixed(data, sizeof(data));

		CMBlock mbSignedData;
		if (BTCKey::ECDSASign(mbChildPrivkey, md, mbSignedData, ECDSA_NID)) {
			bool bVerify = BTCKey::ECDSAVerify(mbChildPubKey, md, mbSignedData, ECDSA_NID);
			REQUIRE(true == bVerify);
		}
	}
}

TEST_CASE("2 Derive PrivateKey and PublicKey for ECDSA CompactSign NID_X9_62_prime256v1", "[BTCKey]") {
	static int ECDSA_NID = NID_X9_62_prime256v1;
