...
namespace Elastos {
	namespace ElaWallet {
		...
		bool PayloadDeploy::Deserialize(const ByteStream &istream, uint8_t version) {
			...
			for (size_t i = 0; i < len; ++i) {
				uint8_t parameter;
				istream.ReadByte(parameter);
				_functionCode.parameters[i] = static_cast<ParameterType>(parameter);
			}

			uint8_t returnType;
			istream.ReadByte(returnType);
			_functionCode.returnType = static_cast<ParameterType>(returnType);

			istream.ReadBytes(_functionCode.codeHash);

			istream.ReadVarString(_name);

			istream.ReadVarString(_version);

			istream.ReadVarString(_author);

			istream.ReadVarString(_email);

			istream.ReadVarString(_description);

			istream.ReadBytes(_programHash);

			istream.ReadVarUint(_gas);
