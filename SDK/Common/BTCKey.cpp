...
namespace Elastos {
	namespace ElaWallet {
		...
		CMBlock
		BTCKey::getDerivePrivKey_depth(const CMBlock &seed, UInt256 &chainCode, const bool useChainCode, const int nid,
									   int depth, ...) {
			CMBlock out;
			if (0 == seed.GetSize()) {
				return out;
		...
		CMBlock
		BTCKey::getDerivePrivKey_Secret_depth(const CMBlock &privKey, UInt256 chainCode, const bool useChainCode,
